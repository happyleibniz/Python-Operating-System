...
def load_textures(self):
    ...
    t = self.texture
    dirs = ['minecraft_python_edition/textures']
    while dirs:
        d = dirs.pop(0)
        ...
        for file in textures:
            if os.path.isdir(d + '/' + file):
                ...
                if ".png" not in file or d == "minecraft_python_edition/textures/blocks/block_destroy":
                    continue
    ...
    for n1, d in items:
        ...
        if d.startswith('minecraft_python_edition/textures/blocks'):
            if d == 'minecraft_python_edition/textures/blocks':
                self.inventory_textures[n] = pyglet.resource.image(f"{d}/{n}.png")
                self.block[n] = t[n], t[n], t[n], t[n], t[n], t[n]
            elif d == 'minecraft_python_edition/textures/blocks/tbs':
                self.inventory_textures[n] = pyglet.resource.image(f"{d}/{n} s.png")
                self.block[n] = t[n + ' s'], t[n + ' s'], t[n + ' b'], t[n + ' t'], t[n + ' s'], t[n + ' s']
            elif d == 'minecraft_python_edition/textures/blocks/ts':
                self.inventory_textures[n] = pyglet.resource.image(f"{d}/{n} s.png")
